...
namespace StatCan.OrchardCore.GitHub.Liquid
{
    public class GetPullRequestFilter : ILiquidFilter
    {
        public async ValueTask<FluidValue> ProcessAsync(FluidValue input, FilterArguments arguments, LiquidTemplateContext ctx)
        {
            ...
            {
                throw new ArgumentException("Services missing while invoking 'github_pullrequest_comments'");
            }
            ...
            {
                throw new ArgumentException("Missing owner, repository or tokenName while invoking 'github_pullrequest_comments'");
            }

            if (!int.TryParse(prNumber, out var parsedNumber))
            {
                throw new ArgumentException("Please provide a valid pull request number while invoking 'github_pullrequest_comments'");
            }
